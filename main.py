    ...
    for panel in panels:
        ...
        for n_holes_f in range(1, 7):
            for n_holes_b in range(1, 7):
                for thickness_f in map(lambda x: x/10000, range(50, 4, -1)):
                    for thickness_b in map(lambda x: x/10000, range(50, 4, -1)):
                        for bolt_diam in map(lambda x: x/1000, range(1, 10)):
                            ...
                            succeeds, mass = eval_iteration(panel_mass, attachment_cfg, attachment_material, fastener_material, spacecraft_thickness)

                            print(calcnumber)
                            calcnumber+=1

                            if succeeds and mass < best_mass:
                                attachment_cfg.name = "iter-0.0." + str(iteration)
